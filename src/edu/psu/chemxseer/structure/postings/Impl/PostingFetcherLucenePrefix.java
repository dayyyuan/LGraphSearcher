...
public class PostingFetcherLucenePrefix implements IPostingFetcherPrefix {
	...
	public PostingFetcherLucenePrefix(String lucenePath, int dbSize,
		...
		try {
			this.luceneSearcher = new IndexSearcher(IndexReader.open(
					luceneDic, true));
			luceneDic.close();
	...
	public int[] getPostingID(int featureID) {
		IGraphFetcherPrefix fetcher = this.getPosting(featureID, new long[4]);
	...
	// TimeComponent[0] for posting fetching
	protected IGraphFetcherPrefix searchIndex(Query query, long[] TimeComponent) {
	...
	public PostingBuilderMem loadPostingIntoMemory(IIndex0 indexSearcher) {
		...
		for (int id : termIDs) {
			mem.insertOnePosting(id, this.getPosting(id, TimeComponent)
					.getOrderedIDs());
		}
		return mem;
	}
