...
/**
 * Goal: offer a generic external-memory sorting program in Java. It must be : -
 * hackable (easy to adapt) - scalable to large files - sensibly efficient.
 * 
...
public class ExternalSort {
	...
	public static List<File> sortInBatch(File file, Comparator<String> cmp,
			boolean containHeader, String[] header) throws IOException {
		List<File> files = new ArrayList<File>();
		BufferedReader fbr = new BufferedReader(new FileReader(file));
		long blocksize = estimateBestSizeOfBlocks(file);// in bytes
		if (containHeader)
			header[0] = fbr.readLine(); // skip the first line
		try {
				...
				while (line != null) {
					...
					while ((currentblocksize < blocksize)
							&& ((line = fbr.readLine()) != null)) { // as long
																	...
																	// have 2MB
						tmplist.add(line);
						currentblocksize += line.length() * 2 + 40; // java uses
																	// 16 bits
																	// per
																	// character
																	// + 40
