...
public class SubSearch_LindexPlus implements ISearcher {
	...
	public SubSearch_LindexPlus(IIndex2 indexSearcher,
	...
	public List<IGraphResult> getAnswer(Graph query, long[] TimeComponent,
		...
		if (maxSubgraphs != null && maxSubgraphs.get(0) == -1) {
			...
			List<Integer> superGraphs = this.indexSearcher.minimalSupergraphs(
					query, TimeComponent, maxSubgraphs);
			// Decide whether to load on-disk Lindex
			// This is For FGindex Only
			...
			if (!loadOnDisk || !onDisk) {
				...
				// }
				// List<GraphResult> onDiskCandidates =
				// this.on_diskFetcher.getJoin(onDiskMaxSubs, TimeComponent);
