...
/**
 * This class stores frequent fragments. It is not guaranteed that a fragment is
 * only once in this set (thus violating the name), this must be ensured by the
 * application. But algorithms like gSpan, FFSM and Gaston ensures this anyway.
 * 
...
public class FragmentSet {
	...
	/*
	 * compares FrequentFragments by looking at the class frequenies and the
	 * fragment size
	 */
	private final static Comparator FREQUENCY_AND_SIZE_COMPARATOR = new Comparator() {
		...
		public int compare(Object o1, Object o2) {
			...
			int diff = ((FrequentFragment) o1).getFragment().getEdgeCount()
					- ((FrequentFragment) o2).getFragment().getEdgeCount();
			if (diff == 0) {
				diff = ((FrequentFragment) o1).getFragment().getNodeCount()
						- ((FrequentFragment) o2).getFragment().getNodeCount();
			}
			return diff;
		}
	};
	 ...
	 * 
	 * @param template
	 *            the fragment set that should be copied
	 */
	...
	/**
	 * Creates a new FragmentSet. The intial size of the set is equal to the
	 * given value.
	 * 
	 * @param size
	 *            the size of the map
	...
	/**
	 * Removes the given fragment from the set. The comparison is done by only
	 * checking the pointer and not for isomorphism, etc.
	 * 
	 * @param fragment
	 *            the fragment that should be removed
	 * @param bin
	 *            the bin in which the fragment resides
	 */
	...
	public void filterClosedFragments() {
		...
		SimpleSubgraphComparator comp = new SimpleSubgraphComparator(
				SimpleNodeComparator.instance, SimpleEdgeComparator.instance);
		for (int i = fragments.length - 1; i >= 0; i--) {
			...
			while (--k >= 0) {
				if (fragments[k] == null)
					continue;
				if (FREQUENCY_COMPARATOR.compare(fragments[i], fragments[k]) != 0)
					break;

				if (comp.compare(fragments[k].getFragment(),
						fragments[i].getFragment()) == 0) {
