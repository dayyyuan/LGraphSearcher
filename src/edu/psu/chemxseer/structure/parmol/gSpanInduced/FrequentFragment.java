...
/**
 * This class represents a discovered frequent fragment. It stores all necessary
 * information for reporting it after the search.
 * 
...
public class FrequentFragment {
	...
	// added by Dayu
	private Graph[] m_coveredGraphs;

	/**
	 * Creates a new FrequentFragment.
	 * 
	 * @param fragment
	 *            the frequent fragments
	 * @param supportedGraphs
	 *            a collection of graphs where the frequent fragments occurs
	 * @param classFrequencies
	 *            the frequencies of the frequent fragment in the different
	 *            classes
	 */
	public FrequentFragment(Graph fragment, Collection supportedGraphs,
			Collection coveredGraphs, float[] classFrequencies) {
		if ((fragment instanceof MatrixGraph)
				&& (fragment.getEdgeCount() / (float) fragment.getNodeCount() < 10)) {
		...
		// added by Dayu
		m_coveredGraphs = new Graph[coveredGraphs.size()];
		// end
		int i = 0;
		...
		// added by Dayu
		i = 0;
		for (Iterator it = coveredGraphs.iterator(); it.hasNext(); i++)
			m_coveredGraphs[i] = (Graph) it.next();
		// end
	 ...
	 * 
	 * @param embeddings
	 *            a collection of embeddings of the frequent fragment into the
	 *            graph database
	 * @param classFrequencies
	 *            the frequencies of the frequent fragment in the different
	 *            classes
	 * @param storeEmbeddings
	 *            <code>true</code> if all embeddings should be stored (they are
	 *            cloned in this case), <code>false</code> otherwise
	 */
	public FrequentFragment(Collection embeddings, float[] classFrequencies,
			boolean storeEmbeddings) {
		if (embeddings.size() > 0) {
			GraphEmbedding emb = (GraphEmbedding) embeddings.iterator().next();

			if (emb.isDirectedGraphEmbedding()) {
				m_fragment = new DirectedListGraph(
						(DirectedGraph) emb.getSubGraph());
			} else {
				m_fragment = new UndirectedListGraph(
						(UndirectedGraph) emb.getSubGraph());
			}
		} else {
			m_fragment = null;
		}
		...
		if (storeEmbeddings) {
			...
			for (Iterator it = embeddings.iterator(); it.hasNext();) {
				m_embeddings[i++] = (GraphEmbedding) ((GraphEmbedding) it
						.next()).clone();
			}
	 ...
	 * @return an array of graphs
	 */
	public Graph[] getCoveredGraphs() {
		if (m_coveredGraphs != null) {
			return m_coveredGraphs;
		} else {
			System.out.println("Sth is Wrong in FrequentFragment~~");
			return null;
		}
	}

	/**
	 * Returns the frequencies of the frequent fragment in the different classes
	...
	/**
	 * Converts this frequent fragment to a string by using the given graph
	 * serializer. The string contains the fragment itself, the class
	 * frequencies and -- if available -- a list of all supported graphs.
	 * 
	 * @param serializer
	 *            a graph serializer
	...
	public String toString(GraphParser serializer) {
		...
		if (m_supportedGraphs != null) {
			for (int i = 0; i < m_supportedGraphs.length; i++) {
				...
				if (i < m_supportedGraphs.length - 1)
					buf.append(',');
			}
			...
			for (int i = 0; i < m_embeddings.length; i++) {
				graphs.put(m_embeddings[i].getSuperGraph(),
						m_embeddings[i].getSuperGraph());
			}
	 ...
	 * <code>Util.getNodePartitions(this.getFragment())</code>. The partitions
	 * are cached for futher reuse.
	 * 
	...
	/**
	 * Returns all embeddings of the fragment, or <code>null</code> if no
